...
export function NewTicketForm() {
  ...
  return <form onSubmit={handleSubmit(submit)} className="overflow-hidden border border-[#deded8] bg-white">
    <div className="border-b border-[#deded8] px-6 py-5 sm:px-8"><p className="font-semibold text-slate-900">Tell us what happened</p><p className="mt-1 text-sm text-slate-500">Clear details help the team respond faster.</p></div>
    <div className="space-y-6 p-6 sm:p-8">
      ...
      <label className="group flex cursor-pointer items-center gap-4 rounded-lg border border-dashed border-slate-300 bg-slate-50/60 p-4 transition hover:border-[#3157d5] hover:bg-blue-50/30"><span className="bg-white p-3 text-[#3157d5]"><FileUp className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-800">{file?.name ?? "Add an attachment"}</span><span className="mt-0.5 block text-xs text-slate-500">PNG, JPG, PDF or TXT up to 5 MB</span></span><input type="file" accept=".png,.jpg,.jpeg,.pdf,.txt" onChange={(event) => setFile(event.target.files?.[0] ?? null)} className="sr-only" /></label>
