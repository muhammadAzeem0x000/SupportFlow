...
export function CommentThread({ ticketId, initialComments, currentUserId }: { ticketId: string; initialComments: CommentRecord[]; currentUserId: string }) {
  ...
  return <section className="overflow-hidden border border-[#deded8] bg-white">
    <div className="flex items-start justify-between gap-4 border-b border-[#deded8] p-5"><div><h2 className="font-semibold text-slate-900">Conversation</h2><p className="mt-1 text-xs text-slate-500">Replies from everyone with access to this ticket.</p></div><span className={cn("inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-[10px] font-bold uppercase tracking-wider ring-1 ring-inset", connectionMeta.className)}><ConnectionIcon className={cn("h-3 w-3", (connection === "connecting" || connection === "recovering") && "animate-spin")} />{connectionMeta.label}</span></div>
    ...
    <div className="max-h-[36rem] space-y-5 overflow-y-auto bg-[#fafaf8] p-5 sm:p-6">{comments.length === 0 && <p className="py-10 text-center text-sm text-slate-400">No replies yet. Start the conversation below.</p>}{comments.map((comment) => { const own = comment.author_id === currentUserId; return <div key={comment.id} className={cn("flex", own ? "justify-end" : "justify-start")}><div className={cn("max-w-[88%] rounded-lg px-4 py-3 sm:max-w-[78%]", own ? "rounded-br-sm bg-[#3157d5] text-white" : "rounded-bl-sm border border-[#deded8] bg-white text-slate-800")}><div className="mb-1.5 flex items-center gap-2 text-[11px] opacity-70"><span className="font-bold">{comment.author?.full_name ?? "SupportFlow user"}</span><span>·</span><time>{formatDate(comment.created_at)}</time></div><p className="whitespace-pre-wrap text-sm leading-6">{comment.body}</p></div></div>; })}</div>
