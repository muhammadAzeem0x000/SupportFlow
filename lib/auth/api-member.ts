...
export async function requireApiMember(roles?: Role[]): Promise<{ member: CurrentMember; supabase: Awaited<ReturnType<typeof createClient>> }> {
  ...
  if (error || !claims?.sub) throw new AppError("UNAUTHENTICATED", "Please sign in to continue.", 401);
  ...
  const member: CurrentMember = { id: data.id, userId: claims.sub, organizationId: data.organization_id, organizationName: (data.organizations as unknown as { name: string } | null)?.name ?? "Organization", fullName: (data.profiles as unknown as { full_name: string } | null)?.full_name ?? "User", email: typeof claims.email === "string" ? claims.email : "", role: data.role as Role, isActive: true };
